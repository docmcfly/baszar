...
//  echo "<pre>".$this->queryString."\n\n"; debug_print_backtrace();  echo "</pre>";
  ...
  public function quote($string, $disable = false) {
    return $disable ? $string : $this->pdo->quote($string);
  }
  ...
  protected function column_push($columns)
  {
    ...
    $stack = array();
    if(strtoupper($columns[0]) === 'DISTINCT'){
       unset($columns[0]);
       $distinct = ' DISTINCT ';
    } else {
       $distinct = '';
  ...
  function get_term($key, $value){
    ...
    $not_quote = isset($match[3]) && $match[3] === '#';
    if (isset($match[4])) {
      if ($match[4] == '') {
         return $this->column_quote($key).'='.$this->quote($value, $not_quote);
      } else {
        // not block
        if ($match[4] == '!') {
    ...
    switch (gettype($value)){
      case 'NULL':
        return $this->column_quote($key).' IS '.$not.$this->quote($value, $not_quote);
        break;

      case 'array':
        if (isset($match[4]) && $match[3] === '<>' && count($value) == 2) {
          return ' ('.$this->column_quote($match[1]).' BETWEEN '.$this->quote($value[0], $not_quote).' AND '.$this->quote($value[1], $not_quote).') ';
        } else {
          return $this->column_quote($match[1]).$not.' IN ('.$this->data_implode($value, ',').') ';
        }
        break;
      case 'boolean':
        $not_quote = strtolower($this->database_type) != 'sqlite';
      ...
      case 'integer':
      case 'double':
        if (isset($match[4]) && $match[4] !== '!' && !$not_quote) {
          return  $this->column_quote($match[1]) . ' ' . $match[4] . ' ' . $this->quote($value, $not_quote). ' ';
        } else {
          return  $this->column_quote($match[1]).$not.'= '.$this->quote($value, $not_quote). ' ';
        }
        break;
    }
  ...
  private function select_query($table, $join, $columns = null, $where = null)
  {
    ...
    // Tools::debug("SET SQL_MODE=ANSI_QUOTES; " .$query);
    return $query;
  }
    ...
    foreach ($datas as $data)
    {
      ...
      foreach ($data as $key => $value)
      {
        switch (gettype($value)) {
          ...
          case 'integer':
          case 'double':
            $values[] = $this->quote($value, $not_quote);
            break;
          case 'boolean':
						$not_quote = strtolower($this->database_type) != 'sqlite';
						$value = $value ? 'true' : 'false';
      ...
      else
      {
        ...
        else
        {
          ...
          switch (gettype($value))
          {
            ...
            case 'integer':
            case 'double':
              $fields[] = $column . ' = ' . $this->quote($value, $not_quote);
              break;
            case 'boolean':
							$not_quote = strtolower($this->database_type) != 'sqlite';
							$value = $value ? 'true' : 'false';
...
//      Tools::debug($statement);
  ...
  {
    return 0 + ($this->query($this->generate_aggregation_query('MAX', $table, $join , $column , $where = null))->fetchColumn());
  }

  public function min($table, $join, $column, $where = null)
  {
    return 0 + ($this->query($this->generate_aggregation_query('MIN', $table, $join , $column , $where = null))->fetchColumn());
  }

  public function avg($table, $join, $column, $where = null)
  {
    return 0 + ($this->query($this->generate_aggregation_query('AVG', $table, $join , $column , $where = null))->fetchColumn());
  }

  public function sum($table, $join, $column, $where = null)
  {
    return 0 + ($this->query($this->generate_aggregation_query('SUM', $table, $join , $column, $where = null))->fetchColumn());
  }
